...
if(isset($_POST['validate'])){
    if(!empty($_POST['firstname']) AND !empty($_POST['lastname']) AND !empty($_POST['login']) AND !empty($_POST['password'])){
        ...
        if($verifUser->rowCount() == 0){
            ...
            $insertUserOnWebsite = $bdd->prepare('INSERT INTO users (firstname, lastname, email, mdp)VALUES (?, ?, ?, ?)');
            // Executer la requete.
            ...
            $getInfosThisUserReq = $bdd->prepare('SELECT id, firstname, lastname, email FROM users WHERE firstname=? AND lastname=? AND email=?');
            $getInfosThisUserReq->execute(array($user_firstname, $user_lastname,  $user_email));
